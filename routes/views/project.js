...
exports = module.exports = async (req, res) => {
	...
	let project = await keystone
		.list("Project")
		.model.findOne({
			projectId: req.params.id,
		})
		.populate(
			"milestones readPermission writePermission output.themecluster measure.field"
		)
		.exec();
	...
	if (!project) {
		req.flash(
			"error",
			"Kein Projekt mit dieser projektId konnte gefunden werden!"
		);
		return view.render("errors/404");
	}

	let milestones = await keystone
		.list("Milestone")
		.model.find({ project: project.id })
		.sort("date")
		.exec();
	let opFields = await keystone
		.list("OperationField")
		.model.find()
		.sort("title")
		.exec();
	let themeclusters = await keystone
		.list("Themecluster")
		.model.find()
		.sort("-title")
		.exec();
	...
	let canWrite =
		req.user.isAdmin ||
		(project.writePermission || []).find((x) => req.user.id == x.id) !==
			undefined;
	let canRead =
		(project.readPermission || []).find((x) => req.user.id == x.id) !==
			undefined || canWrite;
	...
	view.on("post", async (next) => {
		...
		try {
			...
			project.administration.institution =
				req.body["administration.institution"];
			...
			milestones.forEach(async (milestone) => {
				...
				milestone.finalthoughts = req.body[key + ".finalthoughts"];
